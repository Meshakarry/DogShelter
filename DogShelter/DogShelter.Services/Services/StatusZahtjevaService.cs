...
namespace DogShelter.Services.Services
{
    public class StatusZahtjevaService : CRUDService<Model.StatusZahtjeva, LookupSearchRequest, Database.StatusZahtjeva, LookupUpsertRequest, LookupUpsertRequest>, IStatusZahtjevaService
    {
        ...
        {
            StatusZahtjevaNazivi.NaCekanju,
            StatusZahtjevaNazivi.Odobren,
            StatusZahtjevaNazivi.Odbijen,
        };
        ...
        {
            var entity = await _context.StatusZahtjevas.FindAsync(id);
            if (entity != null && CanonicalNazivi.Contains(entity.Naziv))
            {
                throw new BusinessException("Ovaj status je dio sistemske logike (obrada zahtjeva za udomljavanje) i ne može biti preimenovan niti obrisan.");
            }
        }
        ...
        public override async Task<Model.StatusZahtjeva> Update(int ID, LookupUpsertRequest request) { await EnsureNotCanonicalAsync(ID); var r = await base.Update(ID, request); InvalidateCache(); return r; }
