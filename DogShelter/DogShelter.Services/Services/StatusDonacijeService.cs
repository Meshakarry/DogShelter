...
namespace DogShelter.Services.Services
{
    public class StatusDonacijeService : CRUDService<Model.StatusDonacije, LookupSearchRequest, Database.StatusDonacije, LookupUpsertRequest, LookupUpsertRequest>, IStatusDonacijeService
    {
        ...
        {
            StatusDonacijeNazivi.NaCekanju,
            StatusDonacijeNazivi.Uspjesna,
            StatusDonacijeNazivi.Neuspjesna,
            StatusDonacijeNazivi.Vracena,
        };
        ...
        {
            var entity = await _context.StatusDonacijes.FindAsync(id);
            if (entity != null && CanonicalNazivi.Contains(entity.Naziv))
            {
                throw new BusinessException("Ovaj status je dio sistemske logike (obrada donacija) i ne može biti preimenovan niti obrisan.");
            }
        }
        ...
        public override async Task<Model.StatusDonacije> Update(int ID, LookupUpsertRequest request) { await EnsureNotCanonicalAsync(ID); var r = await base.Update(ID, request); InvalidateCache(); return r; }
